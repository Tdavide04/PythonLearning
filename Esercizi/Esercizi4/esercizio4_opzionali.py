...
def school_grading_system(name: str, scores: list):
    """
    This function takes a student's name and their scores.
    The function calculates the average score and print:
    the student's name, the average score and a message inticating if the student passed the exam.
    """
    ...
    if average_score >= 60:
        ...
        result = "Fail"
    print(f"Student: {name}, Average Score: {average_score:.2f}, Result: {result}")

students = [
    {"name": "Alice", "scores": [75, 80, 90]},
    {"name": "Bob", "scores": [55, 60, 45]},
    {"name": "Charlie", "scores": [80, 70, 65]}
    ]
for student in students:
    school_grading_system(student["name"], student["scores"])

'''
name = input("write here your name: ")
scores_input = (input("write here your scores: "))           #tentativo di non usare una lista gia creata ma tramite input
scores_list = scores_input.split(", ")
scores = [int(numero) for numero in scores_input]
student = [name, scores]
school_grading_system(student[name], student[scores])
'''

# -> Student: Alice, Average Score: 81.67, Result: Pass
# Student: Bob, Average Score: 53.33, Result: Fail     
# Student: Charlie, Average Score: 71.67, Result: Pass 
