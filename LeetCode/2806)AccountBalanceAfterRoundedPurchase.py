...
class Solution:
    def accountBalanceAfterPurchase(self, purchaseAmount: int) -> int:
        self.balance = 100
        if purchaseAmount % 5 == 0: # Check if the purchase amount is a multiple of 5
            roundedAmount = round(purchaseAmount + 1, -1) # If it is, round up to the nearest 10 by adding 1 (becuase 5 is rounded down) 
        else:
            roundedAmount = round(purchaseAmount, -1) # Otherwise, just round the purchase amount to the nearest 10
        self.balance -= roundedAmount
        return self.balance
...
if __name__ == "__main__":
    ...
    sos = Solution()
    print(sos.accountBalanceAfterPurchase(14))